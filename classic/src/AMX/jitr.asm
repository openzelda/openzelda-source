...
; You should set this to 0 only when you are secure that there are no range
...
amxhead_s   STRUC
    ...
    _num_publics DW ? ; number of items in the "public functions" table
    _publics	DD ?  ; offset to the "public functions" table
    _num_natives DW ? ; number of items in the "native functions" table
    _natives	DD ?  ; offset to the "native functions" table
    _num_libraries DW ? ; number of items in the "library" table
    _libraries  DD ?  ; offset to the "library" table
    _h_reserved DW ?  ; to round the header size to a multiple of 4 bytes
amxhead_s   ENDS
...
;
GO_ON   MACRO	from, to, opsize	;opsize may be 4, 8 or 12
...
;nop;
	...
	mov	[ebx],edi
        and     eax,0fffffffch
	jnz	op_fill_goon
	;nothing to fill here: ignore this opcode
	add	ebx,8
	cmp	ebx,DWORD ptr [end_code]
	jae	code_gen_done
	jmp	DWORD ptr [ebx]
    op_fill_goon:
    	mov	DWORD ptr [j_fill+1],eax
	GO_ON	j_fill, OP_HALT, 8

    j_fill:
        mov     ebp,12345678h	; get byte count
        push    edx
        add     edx,edi
    fill_loop:
        mov     [edx+ebp-4],eax
...
; fill is still inline...
JIT_OP_FILL:			;length of block to fill is already in ECX
	ret
...
;
; A list of the "run-time-library" functions that are called via idirect calls.
